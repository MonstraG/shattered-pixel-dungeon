...
public enum Document {
	...
	public Image pageSprite(String page) {
		if (page.isEmpty() || !isPageFound(page) || this != ADVENTURERS_GUIDE) {
			...
			return switch (page) {
				default -> new ItemSprite(ItemSpriteSheet.MASTERY);
				case "Examining" -> Icons.get(Icons.MAGNIFY);
				case "Surprise_Attacks" -> Icons.get(Icons.SNAKE);
				case "Identifying" -> new ItemSprite(new ScrollOfIdentify());
				case "Food" -> new ItemSprite(ItemSpriteSheet.PASTY);
				case "Dieing" -> new ItemSprite(ItemSpriteSheet.TOMB);
				case Document.GUIDE_SEARCHING -> Icons.get(Icons.MAGNIFY);
				case "Strength" -> new ItemSprite(ItemSpriteSheet.GREATAXE);
				case "Upgrades" -> new ItemSprite(ItemSpriteSheet.RING_EMERALD);
				case "Looting" -> new ItemSprite(ItemSpriteSheet.CRYSTAL_KEY);
				case "Levelling" -> Icons.get(Icons.TALENT);
				case "Positioning" -> new ItemSprite(ItemSpriteSheet.SPIRIT_BOW);
				case "Magic" -> new ItemSprite(ItemSpriteSheet.WAND_FIREBOLT);
			};
		}
	}
