...
public class ShopRoom extends SpecialRoom {
	...
	protected static ArrayList<Item> generateItems() {
		...
		if (hourglass != null && hourglass.isIdentified() && !hourglass.cursed) {
			int bags = switch (Dungeon.depth) {
				case 6 -> (int) Math.ceil((5 - hourglass.sandBags) * 0.20f);
				case 11 -> (int) Math.ceil((5 - hourglass.sandBags) * 0.25f);
				case 16 -> (int) Math.ceil((5 - hourglass.sandBags) * 0.50f);
				case 20, 21 -> (int) Math.ceil((5 - hourglass.sandBags) * 0.80f);
				default -> 0;
