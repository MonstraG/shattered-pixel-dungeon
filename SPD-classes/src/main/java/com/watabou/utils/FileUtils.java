...
public class FileUtils {
	...
	public static FileHandle getFileHandle(Files.FileType type, String basePath, String name) {
		return switch (type) {
			case Classpath -> Gdx.files.classpath(basePath + name);
			case Internal -> Gdx.files.internal(basePath + name);
			case External -> Gdx.files.external(basePath + name);
			case Absolute -> Gdx.files.absolute(basePath + name);
			case Local -> Gdx.files.local(basePath + name);
			default -> null;
		};
	}
